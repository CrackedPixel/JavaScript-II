...
let fullName = [];
//runners.map(runnerID => fullName.push(runnerID.first_name + ' ' + runnerID.last_name));
...
let allCaps = [];
runners.map(runnerID => allCaps.push(runnerID.first_name.toUpperCase()));
console.log(allCaps); 
...
ticketPriceTotal = runners.reduce((priceCounter, runnerID, currentIndexNum, runners) => {
    return priceCounter += runnerID.donation;
}, 0 );
